...
with mp_hands.Hands(
  ...
  while cap.isOpened():
    ...
    rect_width = 200
    rect_height = 100
    rect_x = int((width - rect_width) / 2)  # X-coordinate of the top-left corner
    rect_y = height - rect_height  # Y-coordinate of the top-left corner
    ...
    text_x = int((width - text_size[0]) / 2)  # X-coordinate of the text
    text_y = rect_y + int((rect_height + text_size[1]) / 2)  # Y-coordinate of the text
    cv2.putText(image, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 2)
    # print(image.shape)
    # print(co)
    if len(co) == 2:
